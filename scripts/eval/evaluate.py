...
def write_evaluation(results: dict):
    ...
    logging.info('Saved eval results to %s:\n%s', output_file, df)
...
def write_submission(results: dict):
    ...
    logging.debug('Saved submission results to %s', output_file)
...
def load_task_configurations(benchmark):
    curr_folder = os.path.dirname(os.path.abspath(__file__))

    try:
        module = importlib.import_module(f"eval.{benchmark}.constants")
    except ImportError:
    ...
    logging.debug(f"Total tasks: {list(tasks.keys())}")
    return tasks
...
def main():
    ...
    jsonl_files = {glob(f"{args.data_dir}/**/*.jsonl")}
    eval_results = {}
    subm_results = {}

    tasks = load_task_configurations(args.benchmark)
    for task, config in tasks.items():
        if f'{task}.jsonl' not in jsonl_files:
            # print(f'Prediction file {task}.jsonl is not found.')
            continue

        logging.info(f'Evaluate task {task}...')
